        ...
        SQL = """
                ...
                FROM (
                    SELECT ID_FOCUS, "data_atendimento", "name", "email", "phone","cpf", "area_pesquisa", "setor","unidade", "especialidade"
                      ...
                      FROM (
                      select 40094 ID_FOCUS, dbamv.fnc_mv_recupera_data_hora(ATENDIME.DT_ATENDIMENTO, atendime.hr_atendimento) "data_atendimento",  paciente.nm_paciente "name", paciente.email "email",
                    ...
                    WHERE paciente.cd_paciente = atendime.cd_paciente
                      ...
                      AND NVL(ATENDIME.CD_TIP_RES,0)  NOT IN (6,5,8,17,13,14) --6 (INTERNAC?O), 5 (CENTRO CIR), 8 OBITO, 17 INTERNAC?O NEFRO, 13 (INTERNAC?O HOSP CANC), 14 (INTERNACAO NEURO).
                ...
                select 40094 ID_FOCUS, dbamv.fnc_mv_recupera_data_hora(DT_ALTA, atendime.hr_alta) "data_atendimento",  paciente.nm_paciente "name", paciente.email "email",
                       (NVL(paciente.NR_DDI_CELULAR, '55') || NVL(paciente.NR_DDD_CELULAR, '') || NVL(paciente.NR_CELULAR, '')) AS "phone",
                       paciente.NR_CPF AS "cpf",
                       'Hospital Anchieta' AS "unidade",
                       DECODE(ATENDIME.TP_ATENDIMENTO,'U','PRONTO_SOCORRO_GERAL','I','INTERNACAO','E','EXAMES','A','AMBULATORIO') "area_pesquisa",
                ...
                WHERE paciente.cd_paciente = atendime.cd_paciente
                  ...
                  AND MOT_ALT.TP_MOT_ALTA <> 'O' -- TODOS MENOS OBITOS
