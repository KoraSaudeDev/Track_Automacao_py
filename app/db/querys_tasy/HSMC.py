...
def DB():
    ...
    try:
        ...
        SQL = """
            ...
            tasy.obter_cpf_pessoa_fisica(a.cd_pessoa_fisica) AS "cpf",
                    case
                    when a.ie_tipo_atendimento = 3 then 'PRONTO_SOCORRO_GERAL'
                    when a.ie_tipo_atendimento = 1 then 'INTERNACAO'
                    when a.ie_tipo_atendimento = 7 then 'HOSPITAL_DIA'
                    when a.ie_tipo_atendimento = 8 and tasy.obter_primeiro_setor_atend(a.nr_atendimento,'C')= 148 then 'ONCOLOGIA'
            ...
            where a.DT_ALTA = TRUNC(TO_TIMESTAMP(:data, 'YYYY-MM-DD HH24:MI:SS.FF3'))
